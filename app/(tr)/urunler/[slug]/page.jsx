...
export function generateMetadata({ params }) {
  ...
  if (!product) return {};
  const catName = T.tr.cats[product.cat];
  return {
    title: `${product.model} | ${catName} | Express Vinç`,
    description: `${product.model} — ${product.h} m, ${catName}. ${T.tr.prodPage.rentPeriods}.`,
    alternates: {
      canonical: abs(productPath("tr", product)),
      languages: {
...
export default function Page({ params }) {
  const product = getProduct(params.slug);
  if (!product) notFound();
  return <ProductDetailPage lang="tr" product={product} />;
}
