...
export function generateMetadata({ params }) {
  ...
  if (!product) return {};
  const catName = T.de.cats[product.cat];
  return {
    title: `${product.model} | ${catName} | Express Vinç`,
    description: `${product.model} — ${product.h} m, ${catName}. ${T.de.prodPage.rentPeriods}.`,
    alternates: {
      canonical: abs(productPath("de", product)),
      languages: {
...
export default function Page({ params }) {
  const product = getProduct(params.slug);
  if (!product) notFound();
  return <ProductDetailPage lang="de" product={product} />;
}
