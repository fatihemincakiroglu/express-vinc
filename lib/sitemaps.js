...
import { ROUTES, r, blogPostPath, productPath } from "./routes";
...
const STATIC_KEYS = [
  ...
  { key: "brands", changefreq: "monthly", priority: "0.8" },
...
export function urlsFor(lang) {
  ...
  for (const product of PRODUCTS) {
    urls.push({
      loc: abs(productPath(lang, product)),
      lastmod: today(),
      changefreq: "monthly",
      priority: "0.7",
      alternates: [
        { hreflang: "tr", href: abs(productPath("tr", product)) },
        { hreflang: "en", href: abs(productPath("en", product)) },
        { hreflang: "de", href: abs(productPath("de", product)) },
        { hreflang: "x-default", href: abs(productPath("tr", product)) },
      ],
    });
  }
